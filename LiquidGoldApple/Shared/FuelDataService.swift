...
/// Loads fuel JSON with a strict fallback order: remote, then app-group cache,
/// then the bundled sample.
///
/// The widget and the app must see the same cache, which is why we write into
/// the app group whenever a remote fetch succeeds.
