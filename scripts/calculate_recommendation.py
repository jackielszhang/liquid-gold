"""Turn a forecast into a fill-now / wait / stay-normal action.

The 30c/L cutoff is deliberate: smaller month-to-month moves are noise relative
to tank size and travel cost, so they should not shout "fill before Wednesday".
"""
...
def calculate_recommendation(
    ...
    if change >= SIGNAL_CENTS:
        # Fill on the last day of the current price window, not on adjustment
        # day itself — pumps usually flip overnight.
        return {
            "action": "fill_now",
    ...
    return {
        ...
        "reason": f"The current {fuel_label.lower()} estimate is within {SIGNAL_CENTS}c/L of flat.",
        "best_fill_up_date": next_adjustment_date,
        "next_price_cents_per_litre": next_price,
        "change_cents_per_litre": change,
    }
