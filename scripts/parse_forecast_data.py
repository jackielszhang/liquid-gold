"""Parse a dated CSV of estimated cents-per-litre moves.

The v1 source is a simple daily feed, not a model. We take the latest row as
the current estimate, then look at the last three petrol values to decide if
the direction has been stable enough to call confidence "high".
"""
...
def _confidence(latest: date, prior: list[Decimal]) -> str:
    # A three-day-old number is already drifting out of this review period.
